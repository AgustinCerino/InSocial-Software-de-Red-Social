...
stContenido cargarContenidoDesdeUser (int idNuevoContenido){
    ...
    fflush(stdin);
    printf("\n");
    printf("Ingrese el id de categoria de la publicacion: ");
    fgets(aux.categoria, sizeof(aux.categoria), stdin);
    fflush(stdin);
    printf("\n");
    aux.activo=1;
    aux.puntosPorCompartir=10; //esta funcionalidad probablemente no se llegue a implementar
    aux.idContenido=idNuevoContenido;
    aux.likes=0;

    printf("Contenido creado con exito!\n");
    system("pause");
    return aux;
