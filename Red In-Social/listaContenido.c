...
void borrarLista(nodoSimple* lista) {
    if (!lista) {
        return;
    }
...
nodoSimple* buscarNodoSimpleSegunCampo(nodoSimple* lista, int idContenido){ //devuelve NULL si no encuentra al nodo o devuelve el nodo si lo encontro
    nodoSimple* seg = lista;
    nodoSimple* aux = NULL;

    if((lista)&& seg->contenidoSocial.idContenido == idContenido)  //caso que fuera el primer nodo
    {
        aux=lista;
